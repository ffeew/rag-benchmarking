...
function EvalRedirect() {
  ...
  // Stop-gap: pull up to 200 datasets to populate the selector. A dedicated
  // "all datasets" endpoint would be cleaner but is a separate follow-up.
  ...
  useEffect(() => {
    if (!isAuthed) {
      void navigate({ ...paths.auth, search: { return: paths.evaluations.to } })
      return
    }
    if (items) {
      ...
      if (target) {
        void navigate({ ...paths.datasetEvaluations(target), replace: true })
      }
    }
  ...
  if (items?.length === 0) {
    return (
      <EmptyState
        className="h-full"
        title="No datasets"
        description="Create a dataset before running evaluations."
        action={
          <Button asChild>
